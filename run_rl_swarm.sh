...
LOG_DIR="$ROOT/logs"
mkdir -p $LOG_DIR
PID_FILE="$LOG_DIR/train_pid.txt"

# Function to start the training process
start_training() {
    local log_file="$LOG_DIR/training_$(date +%Y%m%d_%H%M%S).log"
    echo -e "${CYAN}Starting training process. Logs will be saved to: $log_file${NC}"
    ...
    local pid=$!
    echo $pid > "$PID_FILE"
    echo -e "${GREEN}Training process started with PID: $pid${NC}"
    echo $pid
}
...
# Start training with auto-restart functionality
echo -e "${CYAN}Starting training with auto-restart functionality...${NC}"
echo -e "${YELLOW}The training will automatically restart if it crashes or becomes unresponsive for 20 minutes${NC}"

# Main loop for auto-restart
while true; do
    current_log_file=""
    TRAIN_PID=$(start_training)
    
    # Double-check the PID from the file
    if [ -f "$PID_FILE" ]; then
        FILE_PID=$(cat "$PID_FILE")
        if [ "$TRAIN_PID" != "$FILE_PID" ]; then
            echo -e "${YELLOW}Warning: PID mismatch. Using PID from file: $FILE_PID${NC}"
            TRAIN_PID=$FILE_PID
        fi
    fi
    
    echo -e "${CYAN}Monitoring training process with PID: $TRAIN_PID${NC}"
    current_log_file=$(ls -t $LOG_DIR/*.log | head -1)
    echo -e "${CYAN}Monitoring log file: $current_log_file${NC}"
    
    # Monitor the process and log file
    while true; do
        # Check if the process is still running
        if ! ps -p $TRAIN_PID > /dev/null 2>&1; then
            echo -e "\n${RED}Training process (PID: $TRAIN_PID) has crashed. Restarting in 10 seconds...${NC}"
            sleep 10
            break
        fi
        
        # Check if the log has been updated in the last 20 minutes
        if [ -f "$current_log_file" ]; then
            last_modified=$(stat -c %Y "$current_log_file" 2>/dev/null || stat -f %m "$current_log_file" 2>/dev/null)
            current_time=$(date +%s)
...
# This point is never reached due to the infinite loop above, unless the script is manually interrupted
